...
class ProductsController extends Controller
{
  ...
  public function getProduct(Request $request, $id)
  {
    ...
    // $damua = DB::table('orders')
    // ->join('order_details', 'order_id', '=', 'id')
    // ->select('orders.*', 'product_detail_id')
    // ->get();
    $damua=0;//chua mua
    //  dd(Auth::user()->id);
    ...
    else 
    {
      ...
      $data = Order::where('user_id', Auth::user()->id)->join('order_details', 'orders.id', '=','order_details.order_id')
      ->join('product_details', 'product_details.id', '=','product_detail_id')
      -> where('product_details.product_id', $id)->first();
      // dd(DB::getQueryLog());
    //   dd($data);
      if ($data!=null) $damua=1;
    }
    // if (orders::join('order_details', 'order_id',  ))
    // dd($damua);
   

    return view('pages.product')->with(['data' => ['advertises' => $advertises, 'product' => $product, 'product_details' => $product_details, 'suggest_products' => $suggest_products, 'product_votes' => $product_votes], 'damua'=>$damua]);
  }
