...
class ProductController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        if ($request->information_details != null) {
            ...
            foreach ($images as $k => $img) {
                ...
                if (Str::containsAll($data, ['data:image', 'base64'])) {
                    ...
                    $img->setAttribute('src', '/public/storage/images/posts/' . $image_name);
                }
        ...
        // $product = Product::select('id', 'producer_id', 'name', 'image', 'sku_code', 'monitor',
        //     'front_camera', 'rear_camera', 'CPU', 'GPU', 'RAM', 'ROM', 'OS', 'pin',
        //     'information_details', 'product_introduction')->where('name', $data->name)->first();
        // if ($product->name != null) {
        //     return redirect()->route('admin.product.index')->with(['alert' => [
        //         'type' => 'failed',
        //         'title' => 'Thất bại',
        //         'content' => 'Thêm sản phẩm trùng tên.',
        //     ]]);
        // } else {
            ...
            $pro = Product::select('id', 'name', 'sku_code')->where('sku_code', $request->sku_code)->first();
            if ($pro == null) {
                $product->save();
            } else {
                return redirect()->route('admin.product.index')->with(['alert' => [
                    'type' => 'failed',
                    'title' => 'Thất bại',
                    'content' => 'Thêm sản phẩm bị trùng mã.',
                ]]);
            }
