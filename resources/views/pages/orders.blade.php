              ...
              <table class="table table-striped table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-center"><a href="{{ route('order_page', ['id' => $order->id]) }}"
                        title="Chi tiết đơn hàng: {{ $order->order_code }}">{{ $order->order_code }}</a></td>
                    ...
                    @elseif($order->payment_method->id==2)
                    <td class="text-center">Đã thanh toán</td>
                    @elseif($order->payment_method->id==1&&$order->status==3)
                    <td class="text-center">Đã thanh toán</td>
                    @endif --}}
                    @if($order->status==1)
                    <td class="text-center"><span class="label label-default">Đang xử lý</span></td>
                    
                    @elseif($order->status==2)
                    <td class="text-center"><span class="label label-info">Đang vận chuyển</span></td>
                    @elseif($order->status==3)
                    <td class="text-center"><span class="label label-success">Đã giao hàng</span></td>
                    @elseif($order->status==-1)
                    <td class="text-center"><span class="label label-danger">Đơn đã bị hủy</span></td>
                    @elseif ($order->status==-2)
                    <td class="text-center"><span class="label label-danger">Đơn chờ hủy</span></td>
                    @endif
                    @if($order->status==1)
                    <td class="text-center">
                      <span>
                        <a href="{{ route('order.cancel', ['id' => $order->id]) }}"
                          class="btn btn-icon btn-sm btn-primary tip" title="Chỉnh Sửa">Yêu cầu hủy đơn
