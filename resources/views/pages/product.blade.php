...
<section class="bread-crumb">
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><a href="{{ route('producer_page', ['id' => $data['product']->producer_id]) }}">{{
          $data['product']->producer->name }}</a></li>
                  ...
                  <div class="price-product">
                    ...
                    <div class="product-{{ $key }}" style="display: none;">
                      ...
                      <div class="product-{{ $key }}">
                        @endif
                        @if($product->promotion_price != null && $product->promotion_start_date <= date('Y-m-d') &&
                          $product->promotion_end_date >= date('Y-m-d'))
                          <div class="sale-price">{{ number_format($product->promotion_price,0,',',',') }}
                            <span>VNĐ</span>
                          </div>
                          <div class="promotion-price">
                            <div class="old-price">Giá cũ: <del>{{ number_format($product->sale_price,0,',',',')
                                }}</del> <span>VNĐ</span></div>
                            <div class="save-price">Giảm: <span>{{ number_format($product->sale_price -
                                $product->promotion_price,0,',',',') }}</span> <span>VNĐ</span></div>
                          </div>
                          @else
                          <div class="sale-price">{{ number_format($product->sale_price,0,',',',') }} <span>VNĐ</span>
                          </div>
                          @endif
                          ...
                          <div class="status">
                            Tình trạng:
                            <span style="color: #f30;">Hết hàng</span>
                            <div class="quantity">
                              <tr>
                                <td>Số lượng:</td>
                                <td>{{ $product->quantity }}</td>
                      ...
                      <div class="select-color">
                        <div class="row">
                          ...
                          <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                            <div class="color-inner {{ $key == 0 ? 'active' : '' }}" product-id="{{ $product->id }}"
                              data-key="{{ $key }}" can-buy="{{ $product->quantity > 0 ? '1' : '0'}}"
                              ...
                              <div class="select-inner">
                                ...
                                @else
                                <div class="image-color"><img
                                    src="{{ Helper::get_image_product_url('not-image.jpg') }}"></div>
                                @endif
                    ...
                    <form action="{{ route('show_checkout') }}" method="POST" accept-charset="utf-8">
                      ...
                      <div class="row">
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                          <button type="submit" class="btn btn-lg btn-gray"><i class="far fa-money-bill-alt"></i> Mua
                            ngay</button>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                          <div class="form-center">
                            ...
                            <input type="text" class="qty input-text" id="qty" name="quantity" size="4"
                              value="{{ $data['product_details'][0]->quantity <= 0 ? 0 : 1 }}" min="0"
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                          <button type="button" data-role="addtocart"
                            ...
                            data-url="{{ route('add_cart') }}"><span class="txt-main"><i
                                class="fa fa-cart-arrow-down padding-right-10"></i> Giỏ hàng</span></button>
                        </div>
                      </div>
                    </form>
                  ...
                  <div id="vote" class="tab-pane fade">
                    <div class="content-vote">
                      ...
                      <div class="text-center">
                        <h3>Đặt hàng để có thể để lại ý kiến và đánh giá của bạn!!!
                        </h3>
                      </div>
                      @endif
                      <div class="show-rate">
                        ...
                        <div class="show-rate-content">
                          <div class="total-rate">
                            ...
                            <div class="total-rate-right">
                              ...
                              <div class="total-user">{{ $data['product_votes']->count() }} <i class="fas fa-users"></i>
                              </div>
                            </div>
                          </div>
                          {{-- <h1>88888{{$damua}}99999</h1> --}}
                          @if($data['product_votes']->isNotEmpty())

                          {{-- {{dd($data['product_votes'])}} --}}
                          <div class="vote-inner">
                            @foreach($data['product_votes'] as $vote)
                            <div class="vote-content">
                              <div class="vote-content-left"><img
                                  src="{{ Helper::get_image_avatar_url($vote->user->avatar_image) }}"></div>
                          ...
                          @else
                          <p class="text-center"><strong>Chưa có lượt đánh giá nào từ người dùng. Hãy cho chúng tôi biết
                              ý kiến của bạn.</strong></p>
                          @endif
                        </div>
                      </div>
                    </div>
              ...
              <div class="infomation-detail">
                ...
                <div class="infomation-content">
                  <ul>
                    ...
                    <li><label>Ram:</label>{{ $data['product']->RAM != 0 ? $data['product']->RAM.' GB' : 'Đang Cập
                      Nhật...' }}</li>
                    <li><label>Bộ Nhớ Trong:</label>{{ $data['product']->ROM != 0 ? $data['product']->ROM.' GB' : 'Đang
                      Cập Nhật...' }}</li>
